...
} = require('/opt/node_modules/@aws-sdk/client-sqs');
const {
  ApiGatewayManagementApiClient,
  PostToConnectionCommand,
} = require('/opt/node_modules/@aws-sdk/client-apigatewaymanagementapi');
...
const apiGateway = new ApiGatewayManagementApiClient({
  ...
  endpoint: `https://${process.env.WEBSOCKET_ID}.execute-api.ap-southeast-1.amazonaws.com/dev`, // Retrieve the API endpoint from environment variables
});
...
const updateEvent = async (rawBody) => {
  try {
    await eventSchema.validate(rawBody, { abortEarly: false });
    const { eventId, title, desc, category, image, date, publish, location } =
      rawBody;
...
const removeEvent = async (id) => {
  try {
    ...
    if (!eventData) {
      return buildResponse(404, `Event with id ${id} is not found!`);
    } else {
...
const createOrder = async (message, sqsUrl) => {
  try {
    ...
    const { sequelize, Ticket, Order, OrderDetail, Payment } =
      await initDatabase();
